...
# ============ 测量配置 ============
PLANE_DISTANCE = 1.2      # 相机到平面的默认距离（米）
...
class DetectionWorldConverter(Node):
    ...
    def _pixel_to_world_relative(self, u, v):
        ...
        流程:
          1. MonocularPlaneMeasurer 将像素转为相机坐标系3D点
          2. CAM_TO_BODY 矩阵转到机体坐标系
          3. 四元数旋转到世界坐标系

        返回: np.array([wx, wy, wz]) 或 None (四元数未就绪时)
        """
        # Step 1: 像素 → 相机坐标系3D点 (委托给外置模块)
        X_cam, Y_cam = self.measurer.pixel_to_world_xy(u, v, self.plane_distance)
        Z_cam = self.plane_distance
